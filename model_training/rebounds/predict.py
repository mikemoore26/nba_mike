...
from model_training.rebounds.model import make_rebounds_baseline
...
def _prepare_feature_matrix(today_feat_df: pd.DataFrame, used_features: list[str], feature_medians: dict[str, float]) -> pd.DataFrame:
    X = today_feat_df.copy()

    missing = [c for c in used_features if c not in X.columns]
    if missing:
        raise ValueError(f"today feature table missing required rebound features: {missing}")
...
def _eligibility_flags(player_pred_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in out.itertuples(index=False):
        ...
        if pd.isna(pred_mean) or pred_mean < 2.0:
            row_reasons.append("pred_mean_lt_2")
