...
def _auto_relax_active_within_days(
    *,
    ...
    relaxed = gap_days + 7
    print(
        f"[FG3] Relaxing active_within_days from {active_within_days} to {relaxed} "
...
def _build_historical_slate_from_gamelogs(
    *,
    ...
    """
    Build the slate for a historical date directly from gamelog records.

    Assumes the prepared history dataframe contains enough columns to identify
    team/opponent and home/away context.
    """
...
def predict_fg3_for_date(
    *,
    ...
    """
    Predict FG3 means for a specific slate date.
    ...
    Live mode:
        if game_date is None, fall back to resolve_matchups().
    """

    history_df = _load_history_df(rebuild_history=rebuild_history)

    # ---------------------------------------------------------
    # HISTORICAL MODE (used by backtests)
    # ---------------------------------------------------------
    ...
    pred_df = predict_fg3_player_means(
        history_df=history_df_pre,
        today_df=today_df,
        model_dir=THREES_MODEL_DIR,
    ).copy()
    ...
    if write_output:
        ...
        out_path = results_dir / "pred_fg3.csv"
